...
from jlweather.jlweather_client import WeatherClient
from jlweather import jlweather_consts
from jlweather.lib import daemon
import sys
...
class Eventer:
    ...
    def show_weather_forecast_message(tray):
        ...
        place = wc.get_place()
        if len(place) == 0:
            return Eventer.show_settings()

        forecast, temperature = wc.reload_weather_info()

        ico = jlweather_consts.weather_types.get(forecast)
        if ico:
    ...
    def show_about():
        ...
        text = ''' Este aplicativo foi constru&iacute;do por <b>Jean Lopes</b><br/>
               ...
               '''
        return box.about(None, QString(title), QString(text))
    ...
    def save_new_locality(widget):
        locality_text = str(widget.locality_input_text.text().toUtf8())
